...
import { Decorator, FlowAction, FlowTaskAction, IRunPromise, NodeType } from './nodebase';
import treeKill = require('tree-kill');
...
function doSubstitution(s: string, t: IFileTargets): IReturns {
    // Substitute things like -json ${O:0}
    // ${O:0} = 0th output file specified
    let inp: number[] = [];
    let out: number[] = [];
    ...
    return new class extends FlowTaskAction {
        constructor() {
            super({
                label: conf.title,
                state: TreeItemCollapsibleState.None,
                icon: new ThemeIcon("circle-slash"),
...
export class SynthRootAction extends FlowAction {
    private decorations: FlowAction[] = [];
    private tasks: FlowAction[] = [];
    private outputFiles: Uri[] = [];
    constructor(man: IcarusProperties) {
        super({
            label: "Synthesize",
            state: TreeItemCollapsibleState.Expanded,
            icon: new ThemeIcon("circuit-board"),
            type: NodeType.SYNTH
        });
        ...
        this.decorations.push(new Decorator({
            ...
            icon: new ThemeIcon("file-submodule"),
            context: "type_flist",
            data: <dct.type_flist> { update: (p: Uri[]) => p.forEach(e => {
                if(path.isAbsolute(e.fsPath)) {
                    man.configuration?.synth.top.push(path.relative(man.wsroot?.fsPath || "", e.fsPath));
                } else {
                    man.configuration?.synth.top.push(e.fsPath);
            ...
            children: man.configuration.synth.top.map((x, i) => new Decorator({
                ...
                state: TreeItemCollapsibleState.None,
                icon: new ThemeIcon("file-code"),
                context: "openable type_litem",
                data: <dct.type_litem & dct.openable> {
                    file: () => path.resolve(man.wsroot?.fsPath || "", x),
                    index: i, array: man.configuration?.synth.top
                }
            }))
    ...
    public get children() { return this.decorations; }
}
...
export class SynthYosysIce40 extends FlowTaskAction {
    ...
    constructor(man: IcarusProperties, t: TaskConfig) {
        ...
        this.decorations.push(
            ...
            new Decorator({
                ...
                state: TreeItemCollapsibleState.None,
                icon: new ThemeIcon("output"),
                description: t.log ? getFriendlyPath(man.wsroot?.fsPath || "", t.log) : "None",
                tooltip: t.log ? path.resolve(man.wsroot?.fsPath || "", t.log || "") : "Do not log",
                context: "type_log type_string",
                data: <dct.type_string & dct.openable> {
                    update: (e: string) => this.cfg.log = e,
                    get: () => t.log,
                    file:() => path.resolve(man.wsroot?.fsPath || "", t.log || "")
                }
            }),
            new Decorator({
                ...
                description: t.runInRoot ? 'Yes' : 'No',
                context: "type_bool",
                data: <dct.type_bool> { toggle: () => { t.runInRoot = !t.runInRoot; } }
            }),
            new Decorator({
                label: "Execute:",
                state: TreeItemCollapsibleState.None,
                ...
                description: parseRun(t.run),
                context: "type_string",
                data: <dct.type_string> { update: (v: string) => t.run = v, get: () => t.run }
            })
        );
        ...
        let opts = new Decorator({
            ...
            context: "type_list",
            type: NodeType.DECORATOR,
            data: <dct.type_list> { update: () => new Promise<void>((resolve) => {
                ice40.pickOption(e => {
                    if(e) {
                        t.args?.push(e);
                        resolve();
            ...
            children: t.args?.map<Decorator>((a, i, r) => {
                ...
                return new Decorator({
                    ...
                    context: "type_litem",
                    tooltip: ice40.getTooltipFromSwitch(a),
                    data: <dct.type_litem> { index: i, array: t.args }
                });
            }) // End children
        }); // End options
        ...
        this.decorations.push(new Decorator({
            ...
            children: outputs.map<Decorator>(i => new Decorator({
                ...
                icon: getIconFromFile(conf.synth.outputs[i]),
                context: "openable",
                data: <dct.openable> { file: () => path.resolve(man.wsroot?.fsPath || "", conf.synth.outputs[i]) }
            }))
        }));
    ...
    public get children(): Decorator[] { return this.decorations; }
    private proc : ChildProcess | undefined;
    public run(): Promise<IRunPromise> | undefined {
        if(this.proc) {
            window.showErrorMessage(`A process is already running: ${this.proc.spawnfile}`);
            return;
        }
        ...
        this.proc.stdout?.on("data", d => globalOutput().append(d));
        this.proc.stderr?.on("data", d => globalOutput().append(d));
        return new Promise<IRunPromise> ((accept) => {
            this.proc?.on("exit", (c, s) => {
                this.proc?.stdout?.destroy();
                this.proc?.stderr?.destroy();
                this.proc = undefined;
